...
// .wrangler/tmp/bundle-Jj1rhP/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-Jj1rhP/middleware-insertion-facade.js
init_modules_watch_stub();
...
var workers_site_default = {
  async fetch(request, env, ctx) {
    ...
    try {
      return await (0, import_kv_asset_handler.getAssetFromKV)({ request, waitUntil: ctx.waitUntil.bind(ctx) });
    } catch (e) {
      try {
        const notFoundRequest = new Request(url.origin + "/404.html", request);
        const res = await (0, import_kv_asset_handler.getAssetFromKV)({ request: notFoundRequest, waitUntil: ctx.waitUntil.bind(ctx) });
